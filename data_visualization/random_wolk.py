from random import choice


class RandomWalk():
    """ A class for generating random walks."""

    def __init__(self, num_points=5000):
        """ Initialize random walks. """
        self.num_points = num_points

        # All walks start at point (0, 0).
        self.x_values = [0]
        self.y_values = [0]


    def fill_walk(self):
        """Calculates all walks of the point. """

        # Steps are generating until the desired length is reached.

        while len(self.x_values) < self.num_points:
            # Determination of direction and length of the path.

            x_direction = choice([1, -1])
            x_distance = choice(range(0, 9))
            x_step = x_direction * x_distance
            
            y_direction = choice([1, -1])
            y_distance = choice(range(0, 9))
            y_step = y_direction * y_distance

            # Deviation of zero displacements.
            if x_step == 0 and y_step == 0:
                continue

            # Calculates next values x and y.
            next_x = self.x_values[-1] + x_step
            next_y = self.y_values[-1] + y_step

            self.x_values.append(next_x)
            self.y_values.append(next_y)
